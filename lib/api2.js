...
export async function fetchAPI(path, urlParamsObject = {}, options = {}) {
  ...
  const mergedOptions = {
    headers: {
      ...
      Authorization: "Bearer " + process.env.TOKEN_STRAPI_API_URL,
    },
    ...options,
  };
...
export async function getAllPosts() {
  const res = await fetchAPI(
    "/articles?populate[0]=blocks,author,cover,category&populate[1]=blocks.file,author.avatar&sort[date]=desc"
  );
